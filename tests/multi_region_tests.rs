...
use aframp_backend::multi_region::{
    ConsistencyLevel, DatabaseConfig, FailoverMode, GlobalLoadBalancer, LoadBalancingStrategy,
    MultiRegionConfig, RegionConfig, RegionStatus, ReplicationMode, RequestContext, RoutingReason,
};
...
async fn test_multi_region_config_default() -> Result<(), anyhow::Error> {
    ...
    assert!(matches!(
        config.database.replication_mode,
        ReplicationMode::Asynchronous
    ));
    assert!(matches!(
        config.database.failover_mode,
...
async fn test_multi_region_config_validation() -> Result<(), anyhow::Error> {
    ...
    let original_statuses: Vec<RegionStatus> =
        config.regions.iter().map(|r| r.status.clone()).collect();
    ...
    config
        ...
        .geo_routing
        .country_routing
        .insert("US".to_string(), "us-east-1".to_string());
    assert!(config.validate().is_ok());
...
async fn test_region_lookup_and_routing() -> Result<(), anyhow::Error> {
    ...
    assert!(active_regions
        .iter()
        .all(|r| matches!(r.status, RegionStatus::Active)));

    // Test geographic routing
    assert!(config.get_region_for_country("US").is_some());
    assert_eq!(
        config.get_region_for_country("US").unwrap().region_code,
        "us-east-1"
    );
    assert!(config.get_region_for_country("GB").is_some());
    assert_eq!(
        config.get_region_for_country("GB").unwrap().region_code,
        "eu-west-1"
    );
    assert!(config.get_region_for_country("NG").is_some());
    assert_eq!(
        config.get_region_for_country("NG").unwrap().region_code,
        "af-south-1"
    );
    ...
    assert_eq!(
        config.get_region_for_country("XX").unwrap().region_code,
        "us-east-1"
    );
...
async fn test_capacity_calculation() -> Result<(), anyhow::Error> {
    ...
    let expected_connections: u32 = config
        .regions
        .iter()
        .map(|r| r.capacity.max_connections)
        .sum();
...
async fn test_compliance_requirements() -> Result<(), anyhow::Error> {
    ...
    for (region, requirements) in &compliance_summary {
        assert!(
            requirements.contains(&"Encryption at Rest".to_string()),
            "Region {} should require encryption at rest",
            region
        );
    }
...
async fn test_geographic_routing() -> Result<(), anyhow::Error> {
    ...
    for (country, expected_region) in test_cases {
        ...
        load_balancer.region_health.insert(
        ...
        if country != "XX" {
            assert_eq!(
                decision.selected_region, expected_region,
                "Country {} should route to {}",
...
async fn test_region_failure_handling() -> Result<(), anyhow::Error> {
    ...
    if let Some(health) = load_balancer.region_health.get("us-east-1") {
        assert!(matches!(
            health.status,
            aframp_backend::multi_region::HealthStatus::Unhealthy
        ));
    ...
    if let Some(health) = load_balancer.region_health.get("us-east-1") {
        assert!(matches!(
            health.status,
            aframp_backend::multi_region::HealthStatus::Healthy
        ));
...
async fn test_infrastructure_configuration() -> Result<(), anyhow::Error> {
    ...
    assert!(matches!(
        config.infrastructure.secret_management.provider,
...
mod performance_tests {
    ...
    async fn benchmark_configuration_operations() -> Result<(), anyhow::Error> {
        ...
        println!(
            ...
            NUM_OPERATIONS * 6,
            duration,
            (NUM_OPERATIONS * 6) as f64 / duration.as_secs_f64()
        );
